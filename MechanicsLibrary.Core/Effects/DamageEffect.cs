...
public class DamageEffect : IEffect
{
    ...
    public void Apply(EffectContext ctx)
    {
        ...
        if (_isDebug)
            Console.WriteLine("Originator Strength = " + Strength);
        float OutgoingDmg = (1 + (Strength / 100f)) * Magnitude;
        if (_isDebug)
            Console.WriteLine("Outgoing Damage = " + OutgoingDmg);
        float PercentMitigatedDmg = OutgoingDmg * (1 - (ctx.Reciever.Stats.Armor / 100f));
        if (_isDebug)
            Console.WriteLine("Damage After Percent Mitigation = " + PercentMitigatedDmg);
        int FlatMitigatedDmg = (int)Math.Round(PercentMitigatedDmg - ctx.Reciever.Stats.Grit);
        if (_isDebug)
            Console.WriteLine("Damage after flat mitigation = " + FlatMitigatedDmg);
        ctx.Reciever.Stats.Health -= FlatMitigatedDmg;
        if (_isDebug)
            Console.WriteLine("New health for reciever = " + ctx.Reciever.Stats.Health);
    }
}
